<x-app-layout>
    ...
    <form method="GET" action="{{ route('sensors') }}" class="max-w-md mx-auto mb-2 mt-1">   
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">ID Equipamento</th>
                    <th scope="col" class="px-6 py-3">Mac Sensor</th>
                    <th scope="col" class="px-6 py-3">Status</th>
                    <th scope="col" class="px-6 py-3">IP Cliente</th>
                ...
                @foreach ($sensors as $sensor)
                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                        <td class="px-6 py-4 flex">
                            <a href="{{ route('sensors.edit', $sensor->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Editar</a>
                            <a href="{{ route('sensors.destroy', $sensor->id) }}" class="font-medium text-red-600 dark:text-red-500 hover:underline ml-2">Excluir</a>
                        </td>
                        <td class="px-6 py-4">{{ $sensor->id }}</td>
                        <td class="px-6 py-4">{{ $sensor->cliente->nome ?? 'N/A' }}</td>
                        <td class="px-6 py-4">{{ $sensor->id_equipamento }}</td>
                        <td class="px-6 py-4">{{ $sensor->mac_sensor }}</td>
                        <td class="px-6 py-4">{{ $sensor->status }}</td>
                        <td class="px-6 py-4">{{ $sensor->ip_cliente }}</td>
                        <td class="px-6 py-4">{{ $sensor->created_at }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
