<x-app-layout>
    ...
    <form method="GET" action="{{ route('freezers') }}" class="max-w-md mx-auto mb-2 mt-1">   
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Min/Set/Max</th>
                    <th scope="col" class="px-6 py-3">Nome Unidade</th>
                    <th scope="col" class="px-6 py-3">Referência</th>
                    <th scope="col" class="px-6 py-3">Detalhe</th>
                    <th scope="col" class="px-6 py-3">Cliente</th>
                    <th scope="col" class="px-6 py-3">Etiqueta Ident</th>
                </tr>
            </thead>
                ...
                @foreach ($freezers as $freezer)
                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                        <td class="px-6 py-4 flex">
                            <a href="{{ route('freezers.edit', $freezer->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Editar</a>
                            <a href="{{ route('freezers.destroy', $freezer->id) }}" class="font-medium text-red-600 dark:text-red-500 hover:underline ml-2">Excluir</a>
                        </td>
                        <td class="px-6 py-4">{{ $freezer->cliente->id }}</td>
                        <td class="px-6 py-4">{{ $freezer->cliente->nome }}</td>
                        <td class="px-6 py-4">{{ $freezer->limite_neg }}, {{ $freezer->setpoint }}, {{ $freezer->limite_pos }}</td>
                        <td class="px-6 py-4">{{ $freezer->nome_unidade }}</td>
                        <td class="px-6 py-4">{{ $freezer->referencia }}</td>
                        <td class="px-6 py-4">{{ $freezer->detalhe }}</td>
                        <td class="px-6 py-4">{{ $freezer->cliente->nome ?? 'N/A' }}</td>
                        <td class="px-6 py-4">{{ $freezer->etiqueta_ident }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
